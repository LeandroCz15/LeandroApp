...
public class UrlGen {
	...
	public static String getUrl(Map<String, Object> options) {
		...
		return UrlGen.generateUrl(path, urlEndpoint, src, queryParams, transformation, transformationPosition, signed,
				expire);
	}

	private static String generateUrl(String path, String urlEndpoint, String src, Map<String, String> queryParameters,
			List<Map<String, String>> transformation, String transformationPosition, boolean signed,
			long expireSeconds) {
		...
		if (null != src) {
			newUrl = buildFullUrl(src, queryParameters, transformation, transformationPosition, signed, expireSeconds,
					config.getPrivateKey(), urlEndpoint);
		} else {
			if ("query".equalsIgnoreCase(transformationPosition)) {
				newUrl = buildFullUrl(urlEndpoint + (path.charAt(0) == '/' ? path.substring(1, path.length()) : path),
						queryParameters, transformation, transformationPosition, signed, expireSeconds,
						config.getPrivateKey(), urlEndpoint);
			} else {
				newUrl = buildPathUrl(path.charAt(0) == '/' ? path : "/" + path, urlEndpoint, queryParameters,
						transformation, transformationPosition, signed, expireSeconds, config.getPrivateKey());
			}
		}
		return newUrl;
	}

	private static String buildPathUrl(String path, String urlEndpoint, Map<String, String> queryParameters,
		...
		if (transformation != null && transformation.size() > 0) {
			...
			for (Map<String, String> map : transformation) {
				...
				for (Map.Entry<String, String> entry : entries) {
					...
					String element = Transformer.getInstance().transform(key)
							+ (value.equalsIgnoreCase("-") ? "" : "-" + value);
					tr.append(element.contains("oi") || element.contains("di")
							? element.replaceFirst("/", "").replaceAll("/", "@@")
							: element);
					if (i < entries.size() - 1) {
						tr.append(",");
					}
		...
		try {
			...
			URI tmpUri = new URI(baseUrl.getScheme(), baseUrl.getUserInfo(), baseUrl.getHost(), baseUrl.getPort(),
					newPath, queryMaker.get(), null);

			if (signed) {
				sign(urlEndpoint, expireSeconds, privateKey, queryMaker, tmpUri);
			}

			newUri = new URI(baseUrl.getScheme(), baseUrl.getUserInfo(), baseUrl.getHost(), baseUrl.getPort(), newPath,
					queryMaker.get(), null);
	...
	private static String buildFullUrl(String src, Map<String, String> queryParameters,
			List<Map<String, String>> transformation, String transformationPosition, boolean signed, long expireSeconds,
	...
	private static void sign(String urlEndpoint, long expireSeconds, String privateKey, QueryMaker queryMaker,
			URI tmpUri) {
